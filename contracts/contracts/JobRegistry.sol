...
contract JobRegistry is Ownable {
    ...
    event CriteriaDeliveryIndexed(uint256 indexed jobId, bytes32 evidenceMerkleRoot, string evidenceURI);
    ...
    function indexCriteriaDelivery(uint256 jobId, JobTypes.CriteriaDeliveryProof calldata proof) external onlyOrderBook {
        criteriaDeliveries[jobId] = proof;
        emit CriteriaDeliveryIndexed(jobId, proof.evidenceMerkleRoot, proof.evidenceURI);
    }
