...
contract OrderBook is Ownable {
    ...
    mapping(uint256 => JobTypes.SuccessCriteria) public jobCriteria;
    mapping(uint256 => mapping(address => uint256)) public bidCriteria;
    mapping(uint256 => JobTypes.CriteriaDeliveryProof) public criteriaDeliveries;

    IJobRegistry public jobRegistry;
    IEscrow public escrow;
    IReputationToken public reputationToken;
    IAgentRegistryView public agentRegistry;
    IValidationOracle public validationOracle;
    ...
    function setValidationOracle(address oracle) external onlyOwner {
        validationOracle = IValidationOracle(oracle);
    }
    ...
    function postJobWithCriteria(
        ...
        require(criteriaCount > 0, "OrderBook: criteria count must be positive");
        ...
        jobCriteria[jobId] = JobTypes.SuccessCriteria({
    ...
    function placeBid(
        ...
        bidId = _placeBidInternal(jobId, price, deliveryTime, metadataURI);
    }

    function placeBidWithCriteria(
        uint256 jobId,
        uint256 price,
        uint64 deliveryTime,
        string calldata metadataURI,
        string calldata responseURI,
        uint256 criteriaBitmask
    ) external returns (uint256 bidId) {
        bidId = _placeBidInternal(jobId, price, deliveryTime, metadataURI);
        bidsById[bidId].responseURI = responseURI;
        bidCriteria[jobId][msg.sender] = criteriaBitmask;
    }
    ...
    function submitDeliveryWithEvidence(
        uint256 jobId,
        bytes32 proofHash,
        ...
        Bid storage bid = bidsById[job.acceptedBidId];
        require(bid.bidder == msg.sender, "OrderBook: not winning agent");
        require(address(validationOracle) != address(0), "OrderBook: validation oracle not set");

        JobTypes.SuccessCriteria storage criteria = jobCriteria[jobId];
        require(criteria.criteriaCount > 0, "OrderBook: no criteria set for job");

        job.status = JobTypes.JobStatus.VALIDATING;
        job.deliveryProof = proofHash;

        criteriaDeliveries[jobId] = JobTypes.CriteriaDeliveryProof({
            evidenceMerkleRoot: evidenceMerkleRoot,
            overallProofHash: proofHash,
            evidenceURI: evidenceURI,
            deliveredAt: block.timestamp
        });

        jobRegistry.updateJobStatus(jobId, JobTypes.JobStatus.VALIDATING);
        ...
        validationOracle.requestValidation(
            jobId,
            validator,
            criteria.passingScore,
            criteria.allRequired,
    ...
    function onValidationComplete(uint256 jobId, bool passed, uint8 score) external {
        require(msg.sender == address(validationOracle), "OrderBook: caller is not validation oracle");
        JobState storage job = jobStates[jobId];
        require(job.status == JobTypes.JobStatus.VALIDATING, "OrderBook: job not validating");

        emit ValidationCompleted(jobId, passed, score);
    ...
    function approveDelivery(uint256 jobId) external {
        ...
        require(job.poster == msg.sender, "OrderBook: not poster");
        require(
            job.status == JobTypes.JobStatus.DELIVERED || job.status == JobTypes.JobStatus.VALIDATING,
            "OrderBook: job not delivered or validating"
        );
    ...
    function approveDeliveryOverride(uint256 jobId) external {
        JobState storage job = jobStates[jobId];
        require(job.poster == msg.sender, "OrderBook: not poster");
        require(
            job.status == JobTypes.JobStatus.DELIVERED || job.status == JobTypes.JobStatus.VALIDATING,
