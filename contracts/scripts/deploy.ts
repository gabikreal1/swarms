...
async function main() {
  ...
  const validationOracle = await retryWithBackoff(() =>
    ethers.deployContract("ValidationOracle", [orderBook.target])
  );
  await retryWithBackoff(() => validationOracle.waitForDeployment());
  console.log(`   ValidationOracle: ${validationOracle.target}`);
  ...
  console.log("   Setting ValidationOracle in OrderBook...");
  await retryWithBackoff(() => orderBook.setValidationOracle(validationOracle.target, txOverrides));
  await delay(3000);

  if (process.env.DEFAULT_VALIDATOR_ADDRESS) {
    console.log("   Setting default validator in OrderBook...");
    await retryWithBackoff(() => orderBook.setDefaultValidator(process.env.DEFAULT_VALIDATOR_ADDRESS!, txOverrides));
    await delay(3000);

    console.log("   Registering default validator in ValidationOracle...");
    await retryWithBackoff(() => validationOracle.registerValidator(process.env.DEFAULT_VALIDATOR_ADDRESS!, txOverrides));
    await delay(2000);
  }
  ...
  const deploymentRecord = {
    ...
    contracts: {
      ...
      ValidationOracle: validationOracle.target
    }
  } as const;
